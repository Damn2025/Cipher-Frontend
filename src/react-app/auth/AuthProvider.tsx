...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const ensureProfileExists = async (sess: Session | null) => {
    ...
    try {
      ...
      // Ignore 409 (conflict) errors - profile already exists, which is fine
      if (insertError && insertError.code !== '23505' && insertError.message?.includes('409')) {
        console.warn('Profile creation warning (non-critical):', insertError.message);
      }
    } catch (error) {
      // If this fails due to RLS/trigger timing/etc, we just leave profile null and let refreshProfile handle it later.
      // 409 conflicts are expected if profile already exists, so we ignore them
      if (error && typeof error === 'object' && 'code' in error && error.code === '23505') {
        // PostgreSQL unique violation - profile already exists, which is fine
        return;
      }
